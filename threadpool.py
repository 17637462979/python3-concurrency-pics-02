    ...
    for i in range(1, images_num + 1):
        ...
        if not collection_image_pages.find_one({'image_page_url': data['image_page_url']}):
            try:
                ...
                logger.error('Image page No.{} [{}] of album {} [{}] already exsits in MongoDB'.format(data['image_idx'], data['image_page_url'], data['album_title'], data['album_url']))
...
def get_image(image_page):
    ...
    if not collection_images.find_one({'image_url': data['image_url']}):
        try:
            ...
            logger.error('Image No.{} [{}] of album {} [{}] already exsits in MongoDB'.format(data['image_idx'], data['image_url'], data['album_title'], data['album_url']))
...
def download_image(image):
    ...
    if not os.path.exists(album_path):
        try:
            ...
            logger.debug('Create album directory [{}]'.format(album_path))
        except FileExistsError as e:  # 多线程可能会报异常
            pass
    # 保存图片到本地
...
def step02():
    ...
    workers = min(32, collection_albums.count_documents({'visited': 0}))
    workers = workers if workers > 0 else 1
    with futures.ThreadPoolExecutor(workers) as executor:
        to_do = []
        # 创建并排定Future
        for album in albums:
            future = executor.submit(get_image_pages, album)
            to_do.append(future)
...
def step03():
    ...
    workers = min(64, collection_image_pages.count_documents({'visited': 0}))
    workers = workers if workers > 0 else 1
    with futures.ThreadPoolExecutor(workers) as executor:
        to_do = []
        # 创建并排定Future
        for image_page in image_pages:
            future = executor.submit(get_image, image_page)
            to_do.append(future)
...
if __name__ == '__main__':
    ...
    step01()
    # Step 02: 从MongoDB获取所有图集页面，开启多线程，每个线程访问一个图集URL, 获取每个图集下面的所有包含真实图片资源的页面URL
    step02()
    # Step 03: 从MongoDB获取图片页面，开启多线程，每个线程访问一个图集页面URL, 获取真实图片资源的URL
    step03()
    # Step 04: 从MongoDB获取真实图片资源的URL，开启多线程，每个线程访问一个真实图片资源的URL, 下载到本地
    step04()
